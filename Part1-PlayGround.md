...
|  데이터 종류  |                             장점                             |                             단점                             |
| :-----------: | :----------------------------------------------------------: | :----------------------------------------------------------: |
|     배열      |   인덱스 값을 미리 알고 있을 경우, 데이터에 신속하게 접근    |                         크기가 고정                          |
|      큐       |       FIFO 접근 방식 (먼저 입력된 데이터가 먼저 출력)        |                        접근속도 느림                         |
|     스택      |      LIFO 접근 방식 (나중에 입력된 데이터가 먼저 출력)       |                        접근속도 느림                         |
|    리스트     |               데이터 삽입 및 삭제 속도가 빠름                |                        검색속도 느림                         |
|  해시테이블   |      키값을 미리 알고 있을 경우, 데이터에 신속하게 접근      |                키값이 없으면 메모리 효율 낮음                |
|      힙       | 매우 신속하게 삽입 및 삭제 가능, 최대 혹은 최소 항목에 대한 접근속도 빠름 |                   다른 요소에 접근이 느림                    |
|    트라이     | 데이터 접근 속도가 매우 빠름, 서로 다른 키값에 대한 충돌 가능성 없음 |          특정 상황에서 해시 테이블보다 속도가 느림           |
|   이진 트리   |    균형 잡힌 트리 구조인 경우, 삽입, 삭제, 검색 속도 빠름    | 삭제 알고리즘 작성이 복잡할 수 있음, 삽입 순서에 영향을 받으며 성능 저하가 될 수 있음 |
| 레드블랙 트리 | 삽입, 삭제, 검색속도 매우 빠르고, 트리는 항상 균형 상태를 유지 | 한계 상황에서 데이터 구조를 운영하므로 구현이 매우 까다로움  |
|     R트리     |  공간적 데이터를 나타날 때 좋으며, 2차원 이상의 구조를 지원  |                  성능 검증이 확실하지 않음                   |
